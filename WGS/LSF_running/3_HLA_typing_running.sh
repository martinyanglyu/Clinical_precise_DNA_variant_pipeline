...
do
      bsub -cwd "$GATK_HOME" -q general -n "$cores" -M "$memory" -G compute-hirbea \
       ...
       -o "$GATK_HOME/output_$(date +%Y%m%d_%H%M%S)_${Sample_ID}_HLAtype.txt" \
       -J "$job_name" \
       -R "span[hosts=1] rusage[mem=$memory]" /bin/bash $PY_function_path/HLAtyping.sh "$tumor_BAM" "$normal_BAM" "$variant_calling_out"
